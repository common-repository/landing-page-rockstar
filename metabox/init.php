...
class cmb_Meta_Box {
    ...
    foreach($meta_box['tabs'] as $tab) {
      ...
      $upload[] = in_array( 'file', $types ) || in_array( 'file_list', $types );
		...
		if ( self::get_object_type() == 'post' ) {
			...
			if ( !empty($upload) && in_array( $pagenow, array( 'page.php', 'page-new.php', 'post.php', 'post-new.php' ) ) ) {
		...
		if ( self::get_object_type() == 'user' ) {
			...
			if ( !empty($upload) && in_array( $pagenow, array( 'profile.php', 'user-edit.php' ) ) ) {
    ...
    echo "</div>"; // end the div#lprs_tabs
	...
	public static function render_group( $args ) {
		...
		$classes         = $field_group->args('showfor') ? ' class="showfor ' . $field_group->args('showfor') . '"' : '';

		echo '<tr'.$classes.'><td colspan="2">';
		
		if($field_group->args('showfor') || $field_group->args('hidefor')) {
		
      echo '<div class="feature_primary_content">';
		
		}
		
		echo '<table id="', $field_group->id(), '_repeat" class="repeatable-group'. $sortable .'" style="width:100%;">';
      ...
      echo "<div class='feature_not_supported'><div style='width: 18%; display: inline-block; text-align: right;'><label style='margin-right: 18px; font-weight: bold; color: #23282d;'>".$desc."</label></div><span class='cmb_metabox_description'>This theme doesn't support this feature</span></div>";
    }
