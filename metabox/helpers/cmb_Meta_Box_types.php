...
class cmb_Meta_Box_types {
  ...
  public function theme_choice() {
  
				  if($handle = opendir(dirname(dirname(dirname(__FILE__))) . "/landertemplates")) {
				  
            while( false !== ($file = readdir($handle)) ) {
              if ($file != "." && $file != ".." && strtolower(substr($file, strrpos($file, '.') + 1)) == 'php') {
                $theme_choices[substr($file,strrpos($file, '-') + 1,-4)] =  dirname(dirname(dirname(__FILE__)));
              }
            }
          ...
          $output = "";
				  ...
				  // add themes here
				  $output .= "<div id='theme_choice_box'>";
				  
				  $output .= "<div id='lprs_theme_filter_box'><div id='lprs_theme_filter_0' class='lprs_theme_filter_choice lprs_theme_filter_active'>All</div><div id='lprs_theme_filter_1' class='lprs_theme_filter_choice'>Squeeze Pages</div><div id='lprs_theme_filter_2' class='lprs_theme_filter_choice'>Thank You Pages</div><div id='lprs_theme_filter_3' class='lprs_theme_filter_choice'>Video</div><div id='lprs_theme_filter_4' class='lprs_theme_filter_choice'>Product</div></div>";
				  				  
				  // grab the theme that has been selected - if none, choose #1
				  $pickedtheme = $this->field->escaped_value() ? $this->field->escaped_value() : $this->field->args( 'std' );
          ...
          foreach($theme_choices as $theme => $dir) {
            ...
            $themeCategory = $themeParts[0];
            $themeNumber = $themeParts[1];
            $dir = substr($dir,strpos($dir,"plugins/") + 8);
            $file = $dir . "/landertemplates/template-" . $theme . ".php";
            
            $output .= '<div class="theme_thumb_box category-' . $themeCategory . '">';
            
            if($pickedtheme == $file) {
            
              $output .= '<img id="'. $file . '" class="themethumbnail active" src="'. plugins_url() . '/' . $dir . '/landertemplates/images/theme_' . $theme . '.png" />';
            
            } else {
            
              $output .= '<img id="'. $file . '" class="themethumbnail" src="'. plugins_url() . '/' . $dir . '/landertemplates/images/theme_' . $theme . '.png" />';
            
            }
            
            //$output .= '<p class="theme_thumb_preview" rel="'.$theme.'">Preview</p>';
            $output .= '</div>';
  ...
  public function embed_code() {
    ...
    $embed_value = $this->field->value();
    $escaped = $this->field->escaped_value( 'esc_textarea' );
  
    $args = $this->parse_args( $args, 'textarea', array(
			'class' => 'cmb_textarea',
			'name'  => $this->_name() . '[textarea]',
			'id'    => $this->_id(),
			'cols'  => 60,
			'rows'  => 18,
			'value' => isset($escaped['textarea']) ? $escaped['textarea'] : "",
			'desc'  => $this->_desc( true ),
		) );
		
		$output .= "<div id='embed_form'><div id='embed_form_input'>";
		
		$output .= sprintf( '<textarea%s>%s</textarea>%s', $this->concat_attrs( $args, array( 'desc', 'value' ) ), $args['value'], $args['desc'] );
    
    if($args['value'] == "") {
    
      $output .= "</div><div id='embed_form_output'><h3>Webform Details</h3><div class='embed_form_elements'><p><em>Please enter your webform code...</em></p></div></div></div>";
      
    } else {
    
      $form = new lprs_form_parser($embed_value['textarea']);
    
      $output .= "</div><div id='embed_form_output'><h3>Webform Details</h3>";
      
      $lprs_render_type = (isset($escaped['lprs_render_type']) && $escaped['lprs_render_type'] == 'exact') ? ' selected' : '';
      
      $output .= "<p><select id='lprs_render_type' name='{$this->_name()}[lprs_render_type]' class='form-control'>";
        $output .= "<option value='choose'>Choose fields to include or hide</option>";
        $output .= "<option value='exact'" . $lprs_render_type . ">Insert exact code</option>";
      $output .= "</select></p>";
      
      if(isset($escaped['lprs_render_type']) && $escaped['lprs_render_type'] == 'exact') {
      
        $output .= "<div class='embed_form_elements' style='display: none;'>";
        
      } else {
      
        $output .= "<div class='embed_form_elements'>";
        ...
        if(!empty($form->forms[0]['formElements'])) {
          ...
          $output .= count($form->forms[0]['stylesheets']) > 0 ? "<input type='checkbox' name='{$this->_name()}[lprs_stylesheets]'></td></tr>" : "<em>None</em></td></tr>";
          
          $output .= "<tr><td>Scripts: [" . count($form->forms[0]['scripts']) . "]</td><td></td><td style='text-align: center;'>";
          
          $output .= count($form->forms[0]['scripts']) > 0 ? "<input type='checkbox' name='{$this->_name()}[lprs_scripts]'></td></tr>" : "<em>None</em></td></tr>";
          ...
          foreach($form->forms[0]['formElements'] as $element) {
            ...
            if(isset($embed_value[urlencode($element['name'])])) {
            
              $is_hidden = (isset($embed_value[urlencode($element['name'])]['hide'])) ? 'checked' : '';
              $is_removed = (isset($embed_value[urlencode($element['name'])]['remove'])) ? 'checked' : '';
            ...
            $output .= "<td style='text-align: center;'><input type='checkbox' name='{$this->_name()}[" . urlencode($element['name']) . "][hide]' {$is_hidden}></td>";
            $output .= "<td style='text-align: center;'><input type='checkbox' name='{$this->_name()}[" . urlencode($element['name']) . "][remove]' {$is_removed}></td>";
